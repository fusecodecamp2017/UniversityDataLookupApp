...
public class UniversityLookupService {
    ...
    public GovDataResponseDTO getUniversitiesBySearchCriteria(String name, String city, String state, String zip, String inStateCostRange, String outStateCostRange, String distance, Pageable pageable, String sort) throws IOException {
        ...
        StringBuffer result = getDataGovResponse(response);
    ...
    public GovDataResponseDTO getUniversityById(long id) throws IOException {

        CloseableHttpClient httpclient = HttpClients.createDefault();

        String dataGovUrl = DataGovUtil.generateDataGovUrlSearchingById(applicationProperties.getApiDataGovUrl(), id);
        HttpGet httpGet = new HttpGet(dataGovUrl);
        httpGet.addHeader("X-Api-Key", applicationProperties.getApiDataGovKey());
        log.info("UniversityLookupService.getUniversitiesBySearchCriteria() - Api.data.gov Url: " + dataGovUrl );
        CloseableHttpResponse response = httpclient.execute(httpGet);
    ...
    private StringBuffer getDataGovResponse(CloseableHttpResponse response) throws IOException {
        ...
        try {
            ...
            if( 200 != response.getStatusLine().getStatusCode() ) {
                throw new HttpResponseException(response.getStatusLine().getStatusCode(), "Failed to retrieve university data");
            }
